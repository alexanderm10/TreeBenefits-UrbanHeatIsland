...
NCEP_TPW <- source('NCEP_TPW.R')
# //cloud mask
cloudmask <- source('cloudmask.R')
# //Normalized Difference Vegetation Index
NDVI <- source('compute_NDVI.R')
# //Fraction of Vegetation cover
FVC <- source('compute_FVC.R')
# //surface emissivity
EM <- source('compute_emissivity.R')
# // land surface temperature
LST <- source('SMWalgorithm.R')
# // --------------------------------------------------------------------------------
  
  COLLECTION <- list(ee$Dictionary({
    'L4': {

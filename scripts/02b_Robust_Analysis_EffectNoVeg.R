...
for(i in 1:nrow(dat.uhi)){
  ...
  png(file.path(path.dat, "cities_full_sdei_v6_noveg", "analysis_all_years", paste0(dat.uhi$NAME[i], "_GAM_qaqc.png")), height=6, width=6, units="in", res=120)
  par(mfrow=c(2,2))
  plot(mod.gam)
  ...
  # -------------------------
  write.csv(dat.city, file.path(path.dat, "cities_full_sdei_v6_noveg", "analysis_all_years", paste0(dat.uhi$NAME[i], "_data_full_analyzed.csv")))
  rm(dat.city)
}
...
write.csv(dat.uhi, "../data_processed/analysis_cities_summary_sdei_v6_noveg.csv", row.names=F)
# --------------------------------------------------------------
